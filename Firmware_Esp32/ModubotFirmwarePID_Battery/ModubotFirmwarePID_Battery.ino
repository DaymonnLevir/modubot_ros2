/*
  ...
  Saídas:
    ...
    BAT:<voltage>             tensão filtrada da bateria em volts
    # ...                    mensagens de estado
*/
#include <Arduino.h>
#include <ctype.h>
...
// ================================================================
// [ADICIONADO - MONITOR DE BATERIA MODUBOT]
//
// Circuito validado:
// Bateria+ -> 100 kOhm -> GPIO32 -> 10 kOhm -> GND ESP32
// Capacitor de 1 uF entre GPIO32 e GND ESP32.
//
// IMPORTANTE:
// o GND inferior do resistor de 10 kOhm e do capacitor deve ser
// o GND da propria ESP32, conforme validado nos testes.
// ================================================================

#define BATTERY_PIN 32

const float BATTERY_R1 = 100000.0f;
const float BATTERY_R2 = 10000.0f;

const float BATTERY_DIVIDER_FACTOR =
    (BATTERY_R1 + BATTERY_R2) / BATTERY_R2;  // 11.0

const float BATTERY_CALIBRATION_FACTOR = 1.000f;

// Uma leitura ADC a cada 20 ms.
const uint32_t BATTERY_SAMPLE_MS = 20;

// Uma mensagem BAT por segundo.
const uint32_t BATTERY_REPORT_MS = 1000;

// Filtro exponencial validado no prototipo.
const float BATTERY_ALPHA = 0.05f;

// [FIM - MONITOR DE BATERIA MODUBOT]
...
static void handleLine(
  ...
  switch (command) {
    ...
    case 'C':
      if (parse2f(text, a, b)) {
        enqueueConfig(CONFIG_TICKS, a, b, 0.0f);
      }
      break;
...
static void communicationTask(void *) {
  ...
  // [ADICIONADO - MONITOR DE BATERIA MODUBOT]
  //
  // Estado local da aquisicao da bateria.
  // Esta logica permanece no Core 0 (Communication) e nao altera
  // a tarefa WheelControl no Core 1.
  // ==============================================================

  uint32_t last_battery_sample_ms = millis();
  uint32_t last_battery_report_ms = millis();

  uint32_t battery_sum_mv = 0;
  uint16_t battery_sample_count = 0;

  float battery_voltage_filtered = 0.0f;
  bool battery_filter_initialized = false;
  ...
  for (;;) {
    ...
    // [ADICIONADO - MONITOR DE BATERIA MODUBOT]
    //
    // Aquisicao nao bloqueante:
    // - uma leitura aproximadamente a cada 20 ms;
    // - media das amostras acumuladas;
    // - uma publicacao BAT por segundo;
    // - nenhum delay adicional foi inserido.
    // ==============================================================

    if (now - last_battery_sample_ms >= BATTERY_SAMPLE_MS) {
      last_battery_sample_ms = now;

      uint32_t adc_mv =
          analogReadMilliVolts(BATTERY_PIN);

      battery_sum_mv += adc_mv;
      battery_sample_count++;
    }

    if (
        now - last_battery_report_ms >= BATTERY_REPORT_MS
        &&
        battery_sample_count > 0
       ) {

      last_battery_report_ms = now;

      float adc_mv_average =
          (float)battery_sum_mv /
          (float)battery_sample_count;

      float vout =
          adc_mv_average / 1000.0f;
      ...
      // O bridge ROS 2 existente continuara sendo o unico dono
      // da /dev/ttyUSB0. Ele republicara esta linha em
      // /modubot/serial_rx.
      Serial.printf(
          "BAT:%.2f\n",
          battery_voltage_filtered
    ...
    if (have_state && now - last_odom_ms >= ODOM_PERIOD_MS) {
      ...
      int32_t delta_left =
          state.ticks_left - last_odom_left;

      int32_t delta_right =
          state.ticks_right - last_odom_right;
    ...
    if (
      ...
      Serial.printf(
          "T %lu %.3f %.3f %.1f %d %.3f %.3f %.1f %d\n",
          (unsigned long)state.timestamp_ms,
          state.setpoint_left,
          state.measured_left,
          state.output_left,
          state.dac_left,
          state.setpoint_right,
          state.measured_right,
          state.output_right,
          state.dac_right
      );
    }
...
void setup() {
  ...
  // ================================================================
  // [ADICIONADO - MONITOR DE BATERIA MODUBOT]
  // Configuracao do ADC usado pelo divisor de tensao.
  // ================================================================
  ...
  attachInterrupt(
      digitalPinToInterrupt(SPEED_L),
      isrSpeedL,
      CHANGE
  );

  attachInterrupt(
      digitalPinToInterrupt(SPEED_R),
      isrSpeedR,
      CHANGE
  );

  motion_queue = xQueueCreate(
      1,
      sizeof(MotionPacket)
  );

  config_queue = xQueueCreate(
      8,
      sizeof(ConfigPacket)
  );

  state_queue = xQueueCreate(
      1,
      sizeof(StatePacket)
  );
  ...
  if (!motion_queue || !config_queue || !state_queue) {
    Serial.println(
        F("# ERRO: não foi possível criar as filas")
    );
  ...
  Serial.println(
      F("# [ModubotFirmwarePID READY]")
  );

  Serial.println(
      F("# controle=Core1/50Hz/prio4 comunicação=Core0/prio2")
  );

  Serial.println(
      F("# W/V | S | M 0/1 | K kp ki kd | F kff dac_min | C ticks | P | G")
  );

  BaseType_t communication_created =
  ...
  if (
      communication_created != pdPASS
      ||
      control_created != pdPASS
     ) {

    brakesSet(true);

    Serial.println(
        F("# ERRO: não foi possível criar as tarefas")
    );
